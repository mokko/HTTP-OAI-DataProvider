...
sub queryHeaders {
	...
	$result=$self->_queryHeaders ($params, $result);
...
#put the metadata munging in separate method just for beauty
sub _queryHeaders {
	my $self=shift;
	my $params=shift;
	my $result=shift;
	...
	if ( $params->{identifier} ) {
		$sql .= qq/identifier = '$params->{identifier}' AND /;
	}
	...
	if ( $i == 0 ) {
		$result->_add_error('noRecordsMatch');
	}
return $result;
}
